...
class Telecredito(models.Model):
	...
	def crear_txt_proveedor(self):
		if not self.fecha:
			raise Warning('No se ha establecido fecha')
		if not self.cuenta_cargo:
			raise Warning('No se ha establecido cuenta bancaria')
	...
	def obtener_detalles_facturas_provedor(self, proveedor, monto_abono):
		...
		if not banco:
			raise Warning('No se ha establecido un banco para el Proveedor %s' % proveedor.display_name)

		if not self.cuenta_cargo.bank_account_id:
			raise Warning('No se ha establecido un banco para el diario %s' % self.cuenta_cargo.name)
